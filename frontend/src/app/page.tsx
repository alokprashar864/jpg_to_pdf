...
export default function Page() {
  ...
  const [settings, setSettings] = useState({ pageSize: 'A4', orientation: 'PORTRAIT', dpi: 150 });
  
  const { status, setStatus, progress, setProgress, message, setMessage, downloadUrl, triggerJobAndListen, cleanupSSE } = useConversion();
  ...
  const startConversion = async () => {
    ...
    try {
      ...
      const res = await fetch('http://localhost:3000/api/v1/conversions', {
        method: 'POST',
      ...
      // Calculate overall progress based on finished uploads (basic implementation)
      ...
      const uploadPromises = images.map((img, i) => {
        ...
        return uploadFileToS3(img.file, target.presignedPutUrl, (percent) => {
            // Can be expanded to track exact bytes across all files
        }).then(() => {
            completedUploads++;
      ...
      // Trigger actual processing & SSE connection
